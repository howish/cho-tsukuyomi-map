...
# --------------------------------------------------------------------------
def write_submaps_js(area_coords, sub_coords, booths):
    booth_area = {b["id"]: b["building"] for b in booths if b["building"]}
    counts = {bid: len([b for b in booths if b["building"] == bid])
              for bid in BUILDINGS}
    cfg = {
        "default": "overview",
        "mode": "auto",
        "tabs": [{"id": "overview", "label": "会場全体図", "image": "map-overview.svg"}]
        + [{"id": bid, "label": BUILDINGS[bid]["label"],
            "image": f"map-{bid}.svg", "count": counts[bid]}
    ...
    js = ("/* Auto-generated by scripts/ops/build_c108_map.py — do not hand-edit. */\n"
          "window.SUBMAPS = " + json.dumps(cfg, ensure_ascii=False, indent=1)
          + ";\n")
    (ROOT / EVENT / "submaps.js").write_text(js, encoding="utf-8")
...
def main():
    ...
    write_submaps_js(area, {"minami": mi_co, "higashi": hi_co, "nishi": ni_co},
                     booths)
    print(f"wrote 4 sub-maps + submaps.js ({len(booths)} circles, "
