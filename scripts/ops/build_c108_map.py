...
def write_maps_js(area_coords, sub_coords):
    ...
    Image URLs carry an explicit ?v=<build epoch> so the dynamically-loaded
    sub-map SVGs are cache-busted like every other asset (bump_cache.py only
    touches index.html; these SVGs aren't referenced there). The query
    changes only when this script regenerates the maps — i.e. when the SVGs
    actually change — so unchanged maps stay cached."""
    v = int(time.time())
    img = lambda bid: f"map-{bid}.svg?v={v}"
    cfg = {"default": {"image": f"map-overview.svg?v={v}", "regions": area_coords}}
    for bid in ("minami", "higashi", "nishi"):
        cfg[bid] = {"image": img(bid), "coords": sub_coords[bid]}
