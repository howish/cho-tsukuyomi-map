...
- **New code**: `scripts/post_mirror.py` (storage layer), `scripts/query_mirror.py` (offline CLI), `scripts/r2_sync.py` (backup integration), `scripts/migrate_raw_to_mirror.py` (one-shot importer for existing dumps)
- **Modified code**: `scripts/pull_timelines.py` — read pull_state, pass `start_time`, write returns to mirror in addition to existing raw JSON dump; `doujin-circle-recon` skill's triage/diff subcommands read from mirror first
