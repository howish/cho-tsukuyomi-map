...
window.EVENT_CONFIG = {
  ...
  // Multi-sub-map system (submaps.js → window.SUBMAPS). map_image is just
  // the initial src; the 展示棟 tab bar swaps it per building. Sub-maps +
  // coords are generated by scripts/ops/build_c108_map.py from data.js.
  // 公式マップは準備会著作物のため非掲載 — 当サイト収録サークルの配置のみの略図。
  map_image: "map-overview.svg",
  map_caption: "展示棟タブで切替 ／ マップ上のサークルをタップで詳細。非公式・当落報告より作図、縮尺は不正確",
