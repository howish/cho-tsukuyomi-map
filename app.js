...
(function() {
  ...
  let currentSearch = '';
  // Sub-map building scope (C108 multi-sub-map; null = all buildings).
  // Set by the submap tab system in "auto" mode; respected by applyFilters.
  let activeAreaScope = null;
  ...
  function applyFilters() {
    ...
    const boothArea = (window.SUBMAPS && window.SUBMAPS.boothArea) || null;
    allCards.forEach(card => {
      ...
      const areaOK = !activeAreaScope || !boothArea ||
        boothArea[card.dataset.boothId] === activeAreaScope;
      const show = filterOK && searchOK && areaOK;
    ...
    const isFiltered = activeFilters.size > 0 || currentSearch || !!activeAreaScope;
    if (stats) {
  ...
  // ── Multi-sub-map system (C108) ───────────────────────────────────────
  // Gated on window.SUBMAPS (generated by build_c108_map.py). A 展示棟
  // tab bar swaps the displayed sub-map image + its BOOTH_COORDS and, in
  // "連動 (auto)" mode, scopes the whole page (cards + filters + map) to
  // that building via activeAreaScope. "全件 (all)" mode swaps only the
  // map view and keeps the full card list. The overview sub-map carries
  // clickable building regions (area-targets) that switch tabs.
  (function initSubmaps() {
    const SUB = window.SUBMAPS;
    if (!SUB || !Array.isArray(SUB.tabs) || !SUB.tabs.length) return;
    const titleEl = document.getElementById('map-section-title');
    if (!titleEl) return;

    let activeTab = SUB.default || SUB.tabs[0].id;
    let mode = SUB.mode === 'all' ? 'all' : 'auto';

    const ctrl = el('div', { class: 'submap-controls' });
    const tabsWrap = el('div', { class: 'submap-tabs', role: 'tablist' });
    const tabButtons = {};
    SUB.tabs.forEach(t => {
      const btn = el('button', {
        type: 'button', class: 'submap-tab', 'data-tab': t.id, role: 'tab',
      });
      btn.innerHTML = escapeHtml(t.label) +
        (t.count != null ? ' <span class="submap-tab-count">' + t.count + '</span>' : '');
      btn.addEventListener('click', () => selectTab(t.id));
      tabButtons[t.id] = btn;
      tabsWrap.appendChild(btn);
    });
    const modeBtn = el('button', {
      type: 'button', class: 'submap-mode-btn',
      title: '連動: 展示棟を選ぶとカード一覧も絞り込み / 全件: マップだけ切替',
    });
    modeBtn.addEventListener('click', () => {
      mode = mode === 'auto' ? 'all' : 'auto';
      updateModeBtn();
      applyScope();
    });
    ctrl.appendChild(tabsWrap);
    ctrl.appendChild(modeBtn);
    titleEl.insertAdjacentElement('afterend', ctrl);

    function updateModeBtn() {
      modeBtn.classList.toggle('all', mode === 'all');
      modeBtn.textContent = mode === 'auto' ? '🔗 連動' : '📋 全件';
      modeBtn.setAttribute('aria-pressed', mode === 'auto' ? 'true' : 'false');
    }
    function applyScope() {
      activeAreaScope = (mode === 'auto' && activeTab !== 'overview')
        ? activeTab : null;
      applyFilters();
    }
    function renderAreaTargets() {
      const svg = document.getElementById('venue-map-overlay');
      if (!svg) return;
      const old = svg.querySelector('g.area-targets');
      if (old) old.remove();
      if (activeTab !== 'overview' || !SUB.areaCoords) return;
      const ns = 'http://www.w3.org/2000/svg';
      const g = document.createElementNS(ns, 'g');
      g.setAttribute('class', 'area-targets');
      Object.entries(SUB.areaCoords).forEach(([area, r]) => {
        const rect = document.createElementNS(ns, 'rect');
        rect.setAttribute('x', r.x); rect.setAttribute('y', r.y);
        rect.setAttribute('width', r.w); rect.setAttribute('height', r.h);
        rect.setAttribute('class', 'area-target');
        rect.setAttribute('data-tab', area);
        g.appendChild(rect);
      });
      svg.insertBefore(g, svg.firstChild);
    }
    function selectTab(id) {
      const tab = SUB.tabs.find(t => t.id === id);
      if (!tab) return;
      activeTab = id;
      Object.values(tabButtons).forEach(b => b.classList.remove('active'));
      if (tabButtons[id]) tabButtons[id].classList.add('active');
      const img = document.getElementById('venue-map');
      if (img) {
        const prev = new URL(img.src, location.href);
        const next = new URL(tab.image, location.href);
        if (!next.search && prev.search) next.search = prev.search;
        img.src = next.pathname + next.search;
      }
      window.BOOTH_COORDS = (SUB.coords && SUB.coords[id]) || {};
      renderClickTargets();
      renderAreaTargets();
      applyScope();
    ...
    const ov = document.getElementById('venue-map-overlay');
    if (ov) ov.addEventListener('click', (e) => {
      const tgt = e.target && e.target.closest &&
        e.target.closest('rect.area-target');
      if (!tgt) return;
      e.stopPropagation();
    ...
    let initial = activeTab;
    try {
      const saved = localStorage.getItem('c108-submap');
      if (saved && SUB.tabs.find(t => t.id === saved)) initial = saved;
    } catch (e) {}
    updateModeBtn();
    selectTab(initial);
  })();
