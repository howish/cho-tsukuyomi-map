...
(function() {
  ...
  // Generic hook for events that swap the map image at runtime (e.g. a
  // per-area map). After an event-specific script sets #venue-map.src +
  // window.BOOTH_COORDS, it calls this to re-draw the click-overlay and
  // refresh highlights. Kept tiny + generic; the swap logic itself lives
  // in the event's own script (see FEATURES.md "event-specific behaviour").
  window.refreshVenueMap = function () {
    renderClickTargets();
    applyFilters();
  };
